...
# 保存先のパス
SAVE_PATH = '/media/shimo/HDD_storage/DataSet/Mini_UCF101'
    ...
    for act in act_list:
        ...
        for gidx, group in enumerate(random.sample(range(1, 25 + 1), NUM_GRP)):
            ...
            for sequence in buf:
                ...
                x_data = sequence[fidx, : , : , : ]
                y_data = sequence[NUM_FRAME // 2, : , : , :].reshape(1,img_ch, HEIGHT, WIDTH)
                np.savez(seq_name, x_data=x_data, y_data=y_data)
                csvfile.writelines(seq_name)
                count += 1
                # break
