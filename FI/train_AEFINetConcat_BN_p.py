...
def main():
    ...
    # 保存ディレクトリ
    # save didrectory
    model_dir_name = 'AEFINet_concat_bn_parallel_opt_{}_ch_{}_fsize_{}'.format(args.opt, args.ch, args.fsize)
   ...
   # prepare model
    model = N.GenEvaluator(N.AEFINetConcatBN(f_size=args.fsize, ch=args.ch))
        # model.to_gpu()
    ...
    model_name = 'AEFINet_concat_bn_opt_{}_ch_{}_fsize_{}.npz'.format(args.opt, args.ch, args.fsize)
    chainer.serializers.save_npz(path.join(model_outdir, model_name), model)

    model_parameter = {
        'name': 'AEFINetConcatBN',
