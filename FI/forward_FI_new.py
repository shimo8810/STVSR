...
from CAEFINet import networks as cn
from AEFINet import networks as an
...
def main():
   ...
   # 個々でネットワークを帰る
    ...
    size = 512
    model = an.AEFINet2(f_size=5, n_ch=8, size=size)
    if args.gpu >= 0:
    ...
    dh = 0
    # 保存用ディレクトリの作成
    save_path = path.join(ROOT_PATH, 'examples', 'result_acrobat_s01', args.model.split('/')[-1].split('.')[0])
    ...
    for i in tqdm(range(0,len(img_list)-1, 2)):
        ...
        data[0] = io.imread(img1_path)[dh:dh+h,dw:dw+w,:]
        data[1] = io.imread(img2_path)[dh:dh+h,dw:dw+w,:]
        ...
        img1 = io.imread(img1_path)[dh:dh+h,dw:dw+w,:]
