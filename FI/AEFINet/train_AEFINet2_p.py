...
def main():
    ...
    # 保存ディレクトリ
    # save didrectory
    model_dir_name = 'AEFINet2_opt_{}_ch_{}_fsize_{}'.format(args.opt, args.ch, args.fsize)
   ...
   # prepare model
    ...
    model = N.AEFINet2(vgg_path=path.join(ROOT_PATH, 'models', 'VGG16.npz'), f_size=args.fsize, n_ch=args.ch, size=64)
    if args.gpu >= 0:
        chainer.cuda.get_device_from_id(args.gpu).use()
        model.to_gpu()
    ...
    # setup trainer
    updater = training.StandardUpdater(train_iter, optimizer, device=args.gpu, loss_func=model.get_loss_func())
    ...
    # # eval test data
    trainer.extend(extensions.Evaluator(test_iter, model, device=args.gpu, eval_func=model.get_loss_func()))
    # dump loss graph
    ...
    model_name = 'AEFINet2_{}_ch_{}_fsize_{}.npz'.format(args.opt, args.ch, args.fsize)
